...
public class MyFirebaseMessaging extends FirebaseMessagingService {
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                for (DataSnapshot childSnap:dataSnapshot.getChildren()){
                    ...
                    if (userRef!=null&&sented.equals(userKey)){
                        if(user.equals("0772223398")){
                          sendAdminNotification(remoteMessage,userKey);
                        }
                        else
    ...
    private void sendNotification(RemoteMessage remoteMessage,String keyUser) {
        ...
        DatabaseReference tutorRef=FirebaseDatabase.getInstance().getReference("User");
    ...
    private void sendAdminNotification(RemoteMessage remoteMessage,String keyUser) {
        String user=remoteMessage.getData().get("user");
        String icon=remoteMessage.getData().get("icon");
        String title=remoteMessage.getData().get("title");
        String body=remoteMessage.getData().get("body");
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                for (DataSnapshot child:dataSnapshot.getChildren()) {
                    if (child.getKey().equals(keyUser)) {
                        ...
                        Intent intent = new Intent(MyFirebaseMessaging.this, LoginActivity2.class);
                        ArrayList<String>chatList=new ArrayList<>();
                        chatList.add(user);
                        chatList.add(keyUser);
                        intent.putStringArrayListExtra("ChatID",chatList);
